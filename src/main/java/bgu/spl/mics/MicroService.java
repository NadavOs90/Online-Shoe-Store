...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Request<?>, Callback> completedCallBackMap;
    ...
    public MicroService(String name) {
        this.name = name;
        callBackMap= new ConcurrentHashMap<String, Callback>();
        completedCallBackMap= new ConcurrentHashMap<Request<?>, Callback>();
    }
    ...
    protected final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {
        ...
        callBackMap.put(type.getName(), callback);
    }
    ...
    protected final <T> boolean sendRequest(Request<T> r, Callback<T> onComplete) {
    	completedCallBackMap.put(r, onComplete);
    ...
    public final void run() {
        ...
        while (!terminated) {
			...
			} catch (InterruptedException e) {ShoeStoreRunner.logger.warning(this.getName()+ " was interrupted while waiting for messages");}
        	if(m.getClass()==RequestCompleted.class)
        		completedCallBackMap.get(((RequestCompleted) m).getCompletedRequest()).call(((RequestCompleted) m).getResult());
        	else
            	callBackMap.get(m.getClass().getName()).call(m);
